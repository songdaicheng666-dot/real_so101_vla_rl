...
def test_sft_config_definition_is_valid_but_runtime_is_explicitly_incomplete() -> None:
    ...
    assert config.dataset.normalization.unnorm_key == "so101_battery_dual_rgb_v2"
...
def test_sft_config_can_become_training_ready_without_changing_schema() -> None:
    ...
    ready_dataset = replace(
        config.dataset,
        repo_id="sdc/so101_battery_dual_rgb_v2",
        root="/datasets/so101_battery_dual_rgb_v2",
    )
    ready_training = replace(
        ...
        eval_steps=500,
        save_steps=500,
        output_dir="outputs/so101_battery_dual_rgb_v2",
    )
...
def test_train_only_normalization_stats_round_trip(tmp_path) -> None:
    ...
    assert set(stats.to_openvla_dict()["so101_battery_dual_rgb_v2"]) == {
        "observation.state",
        "action",
    }
