...
- [ ] 连接真实 leader/follower 后验收完整录制、异常丢弃及人工确认流程。
...
当前完整测试结果为 `107 passed`；Ruff 静态检查通过。测试范围如下：
...
| 实体相机档案 | profile 严格解析、by-id/index0、USB身份、Orbbec Context 生命周期、工厂标定哈希和 Dataset 防混录 |
...
1. 完成低速 leader→follower 遥操作和 home pose 可达性检查；`max_relative_target` 按当前决定暂保持 `null`。
2. 在开始长时录制前执行10分钟双相机 soak test，确认有效率至少99%、稳定约30 FPS 且 p95 不超过25 ms。
3. 使用四色方块录制首条任务 episode，验证人工接受/丢弃、整段失败清理和动作方向。
4. 将第一批成功真机示范输入现有适配器，打印原始样本和模型 batch，复核夹爪方向、动作范围及 episode 尾部 mask。
