...
- [ ] 定义单块任务和三块序列任务的机器可读格式。
- [ ] 定义颜色枚举：`red`、`blue`、`yellow`、`green`。
- [ ] 定义回合的 `success`、`failure`、`timeout` 和 `abort` 条件。
...
- [ ] 为单块任务生成统一语言指令，例如 `Pick up the red battery and place it in T0.`。
- [ ] 为序列任务生成三个带目标位次的子任务指令。
- [ ] 决定训练使用的指令语言；如果比赛运行时使用中文指令，则数据中同时覆盖中文表达。
...
- [ ] 记录失败类型，但不将未标注的失败轨迹混入监督学习成功数据。
- [ ] 实现轨迹回放、图像预览、动作曲线和时间同步检查工具。
- [ ] 按场景组合划分训练集、验证集和测试集，避免同一条或近似重复轨迹跨集合泄漏。
...
- [ ] 定义 OpenVLA 输入：相机图像、六维关节状态和语言子任务。
- [ ] 将 LeRobotDataset 转换或直接加载为 OpenVLA-OFT 所需的数据结构。
- [ ] 将 SO-101 的六维绝对关节位置动作映射到模型动作头。
- [ ] 计算并保存各关节动作及状态的归一化统计量。
- [ ] 配置 action chunk 长度、实际执行长度和控制频率。
